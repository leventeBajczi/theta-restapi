...
class GnuLinuxExecutableUtils(@Autowired val config: ApplicationConfiguration) : ExecutableUtils {
    ...
    override fun getAllExecutableVersions(s: String): Map<String, File> {
        synchronized(sync) {
            ...
            for (version in File(config.executables).listFiles(FileFilter {
                it.name.startsWith(prefix) && it.isDirectory && !it.name.equals(prefix)
            }) ?: arrayOf()) {
    ...
    override fun getArchive(s: String, version: String?): File {
        synchronized(sync) {
            val path = if (version == null) {
                config.executables +
                        File.separator +
            ...
            val tmpFile = File.createTempFile(s, ".zip", File(config.tmp))
            try {
                tmpFile.delete()
                arrayOf("bash", "-c", "cd $path && zip -r ${tmpFile.absolutePath} *").runCommand(
                    File(config.tmp),
                    shouldThrow = true
                )
            } catch (e: Exception) {
